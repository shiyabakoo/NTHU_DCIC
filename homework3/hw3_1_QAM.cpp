...
std::vector<double> AutocorrelationFunction(std::vector<Complex>& signal) {
    ...
    std::vector<double> peak_point(100000, 0.0); // 用來儲存每個 group 的 max_gamma_d
        ...
        for(int d = 0; d < cp_length + N; d++) {
            ...
            for(int m = 0; m < 10; m++) {
                for(int k = 0; k < C; k++) {
                    ...
                    phi_d += signal[idx] * std::conj(signal[idx + N]); // 計算 phi_d
                    p_d += std::norm(signal[idx + N]); // 計算 p_d
                }
            }

            double gamma_d = std::norm(phi_d) / (p_d * p_d); // 計算 gamma_d
...
//--------------------------------------------------------------------------------------------------------------------------//
// 計算誤差函式
double CalculateAverageError(const std::vector<double>& fixed_point_gamma, const std::vector<double>& floating_point_gamma, int scalingFactor) {
    double totalError = 0.0;
    size_t size = std::min(fixed_point_gamma.size(), floating_point_gamma.size());
    for (size_t i = 0; i < size; ++i) { 
        double error = std::abs(fixed_point_gamma[i] - floating_point_gamma[i]);
        totalError += error;
    }
    return totalError / size; // 計算平均誤差
}
//--------------------------------------------------------------------------------------------------------------------------//
...
int main(){
    ...
    // std::vector<double> BER_fixed_16bit_values;
     ...
     //for (int run = 0; run < 1000; ++run) {
        ...
        // std::vector<Complex> noise_ofdm_symbols_floating = fixedToFloatingPoint(noise_ofdm_symbols_16bit_fixed_point, scalingFactor_16bit);
        // for (int i = 0; i < 10 && i < noise_ofdm_symbols_16bit_fixed_point.size(); ++i) {
        //     std::cout << "Element " << i << ": " 
        //             << noise_ofdm_symbols_16bit_fixed_point[i].real() << " + "
        //             << noise_ofdm_symbols_16bit_fixed_point[i].imag() << "i" << std::endl;
        // }
        //printComplexVector(noise_ofdm_symbols_floating, "noise_ofdm_symbols_floating");
        // printComplexVector(noise_ofdm_symbols, "noise_ofdm_symbols");

    std::ofstream errorFile("QAM_percentage_error_results.txt");
