    ...
    for(int d = 0; d < cp_length + N; d++){
        ...
        for(int m = 0; m < nb_symbols; m++){
            for(int k = 0; k < C ; k++){
                ...
                // if(d == 0 && m < 10){
                //     std::cout << "Element " << k + m * k << ": (" << std::real(phi_d) << ", " << std::imag(phi_d) << ")\n";
                // }
            }
        }

        // std::cout << "d = " << d << ", phi_d (floating point): (" 
        //           << std::real(phi_d) << ", " << std::imag(phi_d) << ")\n";
        // std::cout << "d = " << d << ", p_d (floating point): (" 
        //       << p_d << ")\n";
        // std::cout << "d = " << d << ", phi_d_norm (floating point): (" 
        //         << std::norm(phi_d) << ")\n";
...
int AutocorrelationFunction_fixed_point(std::vector<ComplexFixed>& signal, int scalingFactor) {
    ...
    for(int d = 0; d < cp_length + N; d++) {
        ...
        // std::cout << "d = " << d << ", phi_d (fixed point): (" 
        //         << fixedToFloat(phi_d.real(), scalingFactor) << ", " << fixedToFloat(phi_d.imag(), scalingFactor) << ")\n";
        // std::cout << "d = " << d << ", phi_d (fixed point): (" 
        //          << phi_d.real() << ", " << phi_d.imag() << ")\n";
...
int main(){
    ...
    // std::vector<double> BER_fixed_32bit_values;
    ...
    for (double SNR_db = 0.0; SNR_db <= 15.0; SNR_db += 3.0) {
        ...
        // std::vector<ComplexFixed> noise_ofdm_symbols_32bit_fixed_point = floatingToFixedpoint(noise_ofdm_symbols, scalingFactor_32bit);
        ...
        // std::cout << "start autocorrelation 32bit fixed point version\n";
        // std::cout << "============================================\n";
        // size_t rx_start_point_32bit_fixed_point = AutocorrelationFunction_fixed_point(noise_ofdm_symbols_32bit_fixed_point, scalingFactor_32bit);
        ...
        // std::vector<ComplexFixed> rx_ofdm_symbols_fixed_32bit = remove_cyclic_prefix_fixed_point(noise_ofdm_symbols_32bit_fixed_point, rx_start_point_32bit_fixed_point);

        // FFT demodulation
        std::cout << "start FFT demodulation\n";
        //std::cout << "rx_ofdm_symbols size: " << rx_ofdm_symbols.size() << std::endl;
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_8bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_8bit, scalingFactor_8bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_10bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_10bit, scalingFactor_10bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_12bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_12bit, scalingFactor_12bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_14bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_14bit, scalingFactor_14bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_16bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_16bit, scalingFactor_16bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_18bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_18bit, scalingFactor_18bit);
        std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_20bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_20bit, scalingFactor_20bit);        
        // std::vector<ComplexFixed> rx_qpsk_symbols_fixed_point_32bit = FFT_demodulation_fixed(rx_ofdm_symbols_fixed_32bit, scalingFactor_32bit);

        // printComplexVector(QPSK_symbols, "QPSK_symbols");
        // printComplexFixedVector(rx_qpsk_symbols_fixed_point, "rx_qpsk_symbols_fixed_point", scalingFactor_16bit);
        ...
        std::vector<bool> rx_bitstream_fixed_point_8bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_8bit, scalingFactor_8bit);
        std::vector<bool> rx_bitstream_fixed_point_10bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_10bit, scalingFactor_10bit);
        std::vector<bool> rx_bitstream_fixed_point_12bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_12bit, scalingFactor_12bit);
        std::vector<bool> rx_bitstream_fixed_point_14bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_14bit, scalingFactor_14bit);
        std::vector<bool> rx_bitstream_fixed_point_16bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_16bit, scalingFactor_16bit);
        std::vector<bool> rx_bitstream_fixed_point_18bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_18bit, scalingFactor_18bit);
        std::vector<bool> rx_bitstream_fixed_point_20bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_20bit, scalingFactor_20bit);
        // std::vector<bool> rx_bitstream_fixed_point_32bit = QPSKdemapping_fixed_point(rx_qpsk_symbols_fixed_point_32bit, scalingFactor_32bit);
        ...
        // double BER_fixed_32bit = calculate_BER(bitstream, rx_bitstream_fixed_point_32bit);
        ...
        // std::cout << " BER_fixed_32bit = " << std::fixed << std::setprecision(10) << BER_fixed_32bit << "\n";
        ...
        // BER_fixed_32bit_values.push_back(BER_fixed_32bit);
    }

     // Output BER vs SNR data to files for MATLAB or Python plotting
    std::ofstream file_floating("ber_floating.txt");
    std::ofstream file_fixed_8bit("ber_fixed_8bit.txt");
    std::ofstream file_fixed_10bit("ber_fixed_10bit.txt");
    std::ofstream file_fixed_12bit("ber_fixed_12bit.txt");
    std::ofstream file_fixed_14bit("ber_fixed_14bit.txt");
    std::ofstream file_fixed_16bit("ber_fixed_16bit.txt");
    std::ofstream file_fixed_18bit("ber_fixed_18bit.txt");
    std::ofstream file_fixed_20bit("ber_fixed_20bit.txt");
    // std::ofstream file_fixed_32bit("ber_fixed_32bit.txt");
